...
def send_news(send_all=False):
    ...
    for title, content, media_url in news:
        try:
            ...
            #response = model.generate_content(input_text)
            #print(response.text)
            if send_media(model.generate_content("Сделай сообщение для телеграм. Только без предлложений ссылок на магазин итд. Но просьба подписаться и репост можно)"+content).text, media_url):  # Здесь отправляем медиа
            ...
            time.sleep(random.randint(100, 3000))  # Пауза перед следующей отправкой
